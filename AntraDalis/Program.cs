...
public class Program
{
    ...
    public static void Main()
    {
        ...
        sw.Start();
        List<List<string>> allRoutes = GenerateAllRoutes(cities.Select(c => c.Name).ToList(), startingCity, routes, cities);
        ...
        Console.WriteLine();
        Console.WriteLine("Best Route by picking locally best goodness: " + string.Join(" -> ", bestRoute));
    ...
    private static List<List<string>> GenerateAllRoutes(List<string> cities, string startingCity, List<Route> routes, List<City> allCities)
    {
        ...
        while (stack.Count > 0)
        {
            ...
            foreach (var route in routes)
            {
                if (route.CityFrom == lastCity)
                {
                    ...
                    {
                        canAdd.Add(route.CityTo);
                    }
                }
            }
            string chosenCity = chooseBestCity(canAdd, currentRoute, allCities);
            if (chosenCity != null) 
            {
                List<string> newRoute = new List<string>(currentRoute);
                newRoute.Add(chosenCity);
                stack.Push(newRoute);
            }
        }
    ...
    private static string chooseBestCity(List<string> canAdd, List<string> currentRoute, List<City> allCities)
    {
        City bestGoodness = null;
        if(canAdd.Count == 0) 
        {
            return null;
        }
        foreach (var city in canAdd)
        {
            City foundCity = allCities.FirstOrDefault(r => r.Name == city);
            if (bestGoodness == null || currentRoute.Contains(bestGoodness.Name) || bestGoodness.GoodnessPoints < foundCity.GoodnessPoints && !currentRoute.Contains(city))
            {
                bestGoodness = foundCity;
            }
        }
        return bestGoodness.Name;
    }
